// Tricubic B2 version with finite differences
...
// Multiply coefficient by B inverse
   ...
   BinvMultiply_B2(tree[panel_index].moments1, tm1);
   BinvMultiply_B2(tree[panel_index].moments2, tm2);
   BinvMultiply_B2(tree[panel_index].moments3, tm3);
    ...
    if (tree[panel_index].MAC < R_sq)
      {
        ...
        Compute_Coeff_B2_fdiff(p_x, p_y, p_z, panel_index);
      }
			...
			size_t panel_index)
{
    ...
    if (tree[panel_index].MAC < R_sq)
      {
        vec_4d tree_result = Call_Tricubic_B2_fdiff(p_x, p_y, p_z,
					      panel_index);
       ...
       for (size_t k = 1; k < size; k++) // skip root
         {
           ...
           BinvMultiply_Transpose_B2(tm1, tree[k].moments1);
           BinvMultiply_Transpose_B2(tm1x, tree[k].dxmoments1);
           BinvMultiply_Transpose_B2(tm1y, tree[k].dymoments1);
           BinvMultiply_Transpose_B2(tm1z, tree[k].dzmoments1);

           BinvMultiply_Transpose_B2(tm2, tree[k].moments2);
           BinvMultiply_Transpose_B2(tm2x, tree[k].dxmoments2);
           BinvMultiply_Transpose_B2(tm2y, tree[k].dymoments2);
           BinvMultiply_Transpose_B2(tm2z, tree[k].dzmoments2);

           BinvMultiply_Transpose_B2(tm3, tree[k].moments3);
           BinvMultiply_Transpose_B2(tm3x, tree[k].dxmoments3);
           BinvMultiply_Transpose_B2(tm3y, tree[k].dymoments3);
           BinvMultiply_Transpose_B2(tm3z, tree[k].dzmoments3);
    ...
    for (int i = 0; i < N_cube; i++)
      {
        ...
        int old_i = particles.old_index[i];
        sweight1  = lambda[0][i];
        sfweight1 = bfac * sweight1;
        sweight2  = lambda[1][i];
        sfweight2 = bfac * sweight2;
        sweight3  = lambda[2][i];
        sfweight3 = bfac * sweight3;
...
// Output to file
    ...
    output_file.open("output_RS_DC_fdiff.txt", ios::out | ios::ate | ios::app);
